...
const html=`<!doctype html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#103c37"><meta name="description" content="LARIAN: Game 3D endless runner original di Kota Nusantara. Jelajahi Nusantara. Kejar Rekor."><title>LARIAN — Jelajahi Nusantara. Kejar Rekor.</title><link rel="icon" type="image/svg+xml" href="/favicon.svg"><style>${style.replaceAll('</style','<\\/style')}</style></head><body><div id="root"></div><script>${js.replaceAll('</script','<\\/script')}</script></body></html>`;
await fs.writeFile('out/index.html',html+'\n<!-- THIRD-PARTY LICENSES\n'+licenses.replaceAll('--','—')+'\n-->');
try {
  await fs.copyFile('public/favicon.svg', 'out/favicon.svg');
} catch {}
try {
  await fs.mkdir('../outputs', { recursive: true });
  await fs.copyFile('out/index.html', '../outputs/LARIAN.html');
} catch {}
console.log(`LARIAN ready: ${(html.length/1024/1024).toFixed(2)} MB in out/index.html`);
